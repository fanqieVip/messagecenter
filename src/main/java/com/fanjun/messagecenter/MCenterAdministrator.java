...
public class MCenterAdministrator extends Thread {
    ...
    private int socketCheckIntervalTime = 10 * 1000;
    private Context context;
    private ConnectionAssistant connectionAssistant;

    public MCenterAdministrator(Context context, BlockingQueue<Msg> messages, Map<String, List<MessageHandler>> messageHanlders) {
        super();
    ...
    public void setSocketCheckIntervalTime(int socketCheckIntervalTime) {
     ...
     */
    public synchronized void connectServer(String host, int port, SocketInterceptor socketInterceptor) {
        if (connectionAssistant != null) {
            connectionAssistant.reboot();
            connectionAssistant = null;
        }
        if (client != null && (!client.isInterrupted() || client.isAlive())) {
            client.disConnect();
            client = null;
        }
        connectionAssistant = new ConnectionAssistant(context, socketCheckIntervalTime, socketInterceptor);
        client = new Client(host, port);
        client.setSocketInterceptor(socketInterceptor);
        client.setConnectionAssistant(connectionAssistant);
        connectionAssistant.start();
        client.start();
    }
    ...
    public void disConnectServer() {
        ...
        if (client != null && (client.isAlive() || !client.isInterrupted())) {
            client.disConnect();
            client = null;
        }
    }
